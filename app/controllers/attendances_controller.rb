class AttendancesController < ApplicationController
  ...
  def edit_change_attendance
    ...
    @users = User.where(id: Attendance.where(name: current_user.name).where(attendance_change_flag: true).where.not(updated_started_at: nil).select(:user_id)).where.not(id: current_user)
