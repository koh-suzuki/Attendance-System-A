class UsersController < ApplicationController
  ...
  def show
    ...
    @notice_users.each do |user|
      @attendances_list = Attendance.where.not(endtime_at: nil).where(overtime_check: false).where(name: current_user.name)
    ...
    # 勤怠変更申請のお知らせ合計
    @att_update_list = Attendance.where(attendance_change_flag: true).where.not(updated_started_at: nil).or(Attendance.where.not(updated_finished_at: nil)).where(name: current_user.name)
